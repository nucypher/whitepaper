...
A possible attack is a user who takes the device colluding with the relevant miners.
...
It would also help to hide the exact pricing information from the mining node while still allowing it to check if the necessary amount was paid using
...
If we use a multi-hop, uni-directional algorithm (LWE is the only one~\cite{lwe-reencryption}), we can have re-encryption keys from the bottom levels to
the top levels~(\figref{fig:hierarchical-pre}).
When access is granted to a directory, all its files and subdirectories have a path to be re-encrypted under the directory key, and then re-encrypted for the
recipient:

...
There is also a possibility to use quantum-resistant NTRU if the need arises~\cite{wiki:ntru,ntrureencrypt}.
...
This combination $(edek, c)$ can be stored anywhere - in IPFS, or S3, or any kind of decentralized or centralized storage.
\begin{figure}
\centering
...
The user who we delegate to is not known in advance, when we are storing the data.
Firstly, the receiver should show the sender his public key~(Fig.~\ref{fig:arch-delegate}).
Often it makes sense to make the public key corresponding to the address in Ethereum network (then it will be shown once a transaction is made from that
...
When the receiver wants to decrypt data shared with him, he firstly downloads that data from the storage or encrypted stream~(Fig.~\ref{fig:arch-decrypt}).
He separates out EDEK from the message and sends $edek$ to the network of re-encryption nodes and finds active re-encryption nodes which can share data

...
For the initial release, we will ensure that the nodes are staying online and correctly re-encrypting the data.
...
This also naturally limits the number of nodes, similar to the mechanism for staking master nodes in the DASH cryptocurrency.
...
\end{itemize}
When performing this ``ping'', one can choose to do only the tests desired.
...
Nodes which currently don't participate in the validation won't be able to distinguish whether they are being challenged or receiving a real client request.

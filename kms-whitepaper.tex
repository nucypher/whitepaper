...
In the first release, we will ensure that the nodes are staying online and correctly re-encrypting the data without requiring
...
This also naturally limits number of nodes, similar to how it is done for staking master nodes in cryptocurrency DASH~\cite{dash:whitepaper}.
...
\end{itemize}
When one performs this ``ping'', he can choose to do only part of the tests if he believes that this part is sufficient to determine availability.
...
For example, if $100,000$ nodes are online and Ethereum block time is $24$~s (current value), health of every node will be checked approximately every $21$
hours.
...
Nodes which currently don't participate in the validation won't be able to distinguish if they are challenged or it's a real client using the system.

It is also not very feasible to do the health check performed by all the nodes.
...
If the nodes are found to be not healthy, they're can be punished by an losing average amount of block rewards they've obtained since the last health check.
...
In addition, the miners have ``availability rewards'' generated if they stay online and always ready to provide their services, being checked with the help of

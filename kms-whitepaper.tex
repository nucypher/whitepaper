...
\title{[Draft 4] \kms: Decentralized key management system}
...
One thing to keep in mind is that it could imply that the actual bulk of the data should pass through re-encryption nodes, not just encrypted symmetric
keys.
...
It is possible to do checks for real re-encryption keys deployed by participants.
In order to do that, the owner of the data can prepare a ``challenge pack'' - the data which doesn't correspond to any sensitive data, but will be used to
challenge the nodes which have re-encryption keys later on.
The ``challenge pack'' consists of the input ciphertexts and expected outputs.
Although, this method works only for proxy re-encryption algorithms which do not have probabilistic outputs.
The recipient of the data can decrypt the challenge pack and demonstrate inability of a re-encryption node to re-encrypt data when it starts happening.

The nodes which challenge other re-encryption nodes are essentially voting which nodes are misbehaving and betting on the result of this vote.
They should be unable to figure out what the results of votes of others are before everyone committed the vote.
...
That's why we'll introduce a rewards schedule where some rewards are ``mined'', asymptotically switching to no inflation as the time goes.

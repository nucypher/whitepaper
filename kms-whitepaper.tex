...
NuCypherKMS enables KMS as a service. It uses a decentralized network to remove the reliance on central service
...
This risk is usually deadly for multi-party computations (such as Enigma~\cite{enigma}), however in our case the attacker only gains the ability to only wrongfully apply re-encyrption policies, not to decrypt data or to grant access to a user who hasn't been granted access to the data.
...
An m-of-n scheme exists for AFGH~\cite{AFGH} encryption, for example.
...
produce a different re-encryption result, and the miner's collateral deposit should be awarded to the challenger.

The system should also produce a number of ``wrong re-encryptions'' by itself, in order to incentivize challengers to operate, as pointed out by
...
The miners are paid for both providing re-encryptions and making themselves available to re-encrypt data.
The miners can be paid either by the owner of the data, or (partly) by users of the data.
...
\kms~can be used for shared credentials that employees use to access web services.
An audit log can be built to monitor what secrets are accessed and by whom.
...
content marketplaces to decentralized Netflix to patient-controlled electronic health records. 

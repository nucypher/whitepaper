...
In network, there are multiple re-encryption nodes which apply access management polocies.
...
The first risk of this model is collusion of a miner and a reader of the data.
...
We use multiple ways of counter-acting this threat: pseudo-anonymity of re-encryption keys, split-key proxy re-encryption and challenge protocol.
On top of those, we apply economic incentives for fair operation, described in Sec.~\ref{sec:token}.

The second risk is nodes starting to malfunction (return fake data instead of performing re-encryptions).
We solve this problem using a challenge protocol.

The third risk is nodes colluding with each other to perform 50\% attacks.
This risk is usually deadly for multi-party computations (such as Enigma~\cite{enigma}), however in our case it gives the attacker ability to only wrongfully apply policies, not to decrypt data or to grant access to a user who never has access to the data.
The system should, ideally, be as much more decentralized as possible, however 50\% attacks don't compromise security of the data, just like 50\% attacks in
proof-of-work cryptocurrencies don't give an attacker an ability to move funds.

\subsection{Pseudo-anonymity}

It is highly beneficial for the system security that re-encryption nodes do not know what is it they are re-encrypting.
One reason for that is that they don't know which re-encryption keys to perform collusion attacks on (and trying to collude with all the network participants
would be infeasible when the network is decentralized).
But also, importantly, pseudo-anonymity of re-encryption keys allows to run a challenge protocol (Sec.~\ref{sec:challenge-protocol}).
...
The nonce should be told the recipient of the data once the data is shared, so that the recipient can form a proper request to apply the re-encryption key
related to the data shared, not the other data.
...
Imagine that a re-encryption node decides to re-encrypt data all the time rather than to apply policies.
Analog to split-key schemes in proxy re-encryption can be used to solve this problem.
...
An m-of-m scheme exists for AFGH~\cite{AFGH} encryption, for example.
...
There is a risk of miners returning just random numbers instead of re-encrypting the actual data.
Since the data is private, users of the system cannot publish this data and their key as a proof that the miner has cheated.
...
If this re-encryption was a challenge and the miner has cheated, the user can present a proof that non-sensitive keys related to this challenge should actually
produce a different re-encryption result, and the miner's collateral deposit should be taken to reward the challengers.

The system should produce a number of ``wrong re-encryptions'' also by itself, in order to incentivise challengers to operate, as was pointed out by
...
Vitally, the token also incentivising correctness of computation and security if the system.

\subsection{Token distribution}

All the \kms~Tokens exist from the time of creation of the system on main net.

Miners can hold a number of re-encryption keys proportional to number of tokens they hold as a collateral deposit, locked up by a smart contract.
The miners are paid for both serving re-encryptions and making themselves available to re-encrypt data.
The miners could be paid either by the owner of the data, or (partly) by users of the data.
The latter is more relevant for a usecase of DAOs based on decentralized DRM.

If miners become unavailable, they lose their ``availability'' for the time period.
...
\subsection{Mobile device management and revocation}
In an enterprise MDM setting, re-encryption tokens can be created for valid devices. When a device is lost or retired,
or an employee leaves the organization, the re-encryption token can be deleted to revoke their access. This avoids the

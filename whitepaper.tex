...
\usepackage[breaklinks,pdftitle={The NuCypher Network: A decentralized cryptological network offering accessible, intuitive, and extensible runtimes and interfaces for secrets management and dynamic access control}, pdfauthor={Michael Egorov},colorlinks,urlcolor=blue,citecolor=darkgreen,linkcolor=darkblue]{hyperref}
\graphicspath{{pdf/}}
...
\begin{abstract}
    ...
    /network~enables developers to manage secrets and dynamically grant and revok access to sensitive data in public networks.
\end{abstract}
...
\network~is a decentralized cryptological network offering accessible, intuitive, and extensible runtimes and interfaces for
secrets management and dynamic access control.
It introduces an extensible, character-based narrative wherein characters are granted cryptological powers.
The first new character introduced, Ursula, uses proxy re-encryption to manage secrets and dynamically grant and revoke access to sensitive data
in a way that cannot be achieved by traditional symmetric or public-key encryption schemes.
Cryptographic tokens are used to coordinate network participants and provide Sybil-resistance.
...
\begin{align}
    dek &= \text{random}();\\
    c &= \text{encrypt}_{sym}(dek, d);\\
    edek &= \text{encrypt}_{pke}(pk_r, dek).
\end{align}
...
For the first version of \network~we choose ECIES for which we created a proxy re-encryption algorithm~\cite{umbral-spec}.
...
Proxy re-encryption allows \network~to split the trust between access management and decryption rights,
...
When \network~is used to secure access to files~(Sec.~\ref{sec:files}) or messages, both granting and revocation of access are important.
...
\network~can be interfaced from a traditional, centralized application.
...
requests the network to transform $edek$ into $edek^{\prime}$,
...
However, it takes time to make a roundtrip with \network.
...
In \network, miners are re-encryption nodes.
...
The mechanism by which we achieve this is \network~Token (NU).
NU is both the reward miners get for contributing work and the price consumers (owners of the data) pay for access to re-encryption services.
...
In principle, it would've been possible to lock Ethers instead of NU token.
...
If NU tokens are used for staking, an attack like that would reduce the value of the tokens together with the value of the network dropping.
...
An interesting property of \network~is that security improves as the number of network participants grows. As additional re-encryption
...
\section{Use cases}
\network~provides the infrastructure for a variety of applications that require sharing of sensitive data as a basic
...
A blind identity management service can be constructed using \network. Identities can be encrypted client-side and stored with the
...
\network~is ideal for the storage of any secrets, such as sensitive environment variables, database credentials, and API keys.
...
\network~can manage shared credentials that employees use to access web services.
...
\subsection{Private use of \network}
In order to benefit traditional enterprises (financial, healthcare, government or IoT companies), \network~can be deployed privately.
...
\section{Summary}
\network~is a decentralized key management service and cryptographic access control layer for the blockchain and
decentralized applications.
Developers and enterprises alike can leverage it to create highly-secure applications in healthcare,
financial services, and more.
By bringing private data sharing and computation to the public blockchain, \network~enables everything from encrypted
